...
public class ImageController {
    ...
    @GetMapping("/{fileName}") // html의 image 태그의 요청 방식은 get 방식
    public ResponseEntity<?> getImage(@PathVariable String fileName) {
        File file = new File(uploadDir + "/" +  fileName); // 디렉토리와 파일명을 합친 File 객체 생성
        ResponseEntity<byte[]> result = null;
        HttpHeaders headers = new HttpHeaders(); // 응답용 헤더 객체 생성

        try {
            // 브라우저에게 응답하는 컨텐트의 타입이 무엇인지를 명시해줌 -> 그래야 브라우저가 명확하게 화면에 표시하는 게 가능
            headers.add("Content-Type", Files.probeContentType(file.toPath()));

            // 전달하고자 하는 파일을 카피한 후 바이트 배열로 변환해서 ResponseEntity에 담아 전달.
